...
const IMac = () => {
    ...
    const getCurrentModel = () => {
        return imacModels.find((model) => model.label.toLowerCase() === selectedColor.toLowerCase());
    }
                ...
                {imacModels.map((model, index) => (
                    <div
                        ...
                        <button
                            ...
                            className={`2xl:w-8 md:w-5 w-4 2xl:h-8 md:h-5 h-4 rounded-full flex items-center bg-[#10505b] ${selectedColor === model.label.toLowerCase() ? 'border-2 border-blue-500' : ''}`}
                            onClick={() => {
                        ...
                        {visibleLabel === model.label && (
                            <span className='text-sm font-semibold text-gray-700'>{model.label}</span>
                        )}
                    </div>
                ))}
